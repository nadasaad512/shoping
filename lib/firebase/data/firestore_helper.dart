...
class FirestoreHelper {
  ...
  addProductToCart(Product product) async {
    ...
    DocumentReference<Map<String, dynamic>> doc = await firebaseFirestore.collection('users').doc(myid).collection('cart').add(product.toMap());
    print(doc);
  }
  Future<List<Product>> getAllCarts() async {
    String myid = FirebaseAuth.instance.currentUser.uid;
    ...
    List<Product> allCarts = allProductsSnapshot.docs.map((e) {
  ...
  addNewProduct(Product product) async {
    DocumentReference<Map<String, dynamic>> doc = await firebaseFirestore.collection('products').add(product.toMap());
    //log(doc.id);
  }
  addProductTocash(Product product) async {
    String myid = FirebaseAuth.instance.currentUser.uid;
    DocumentReference<Map<String, dynamic>> doc = await firebaseFirestore.collection('users').doc(myid).collection('cash').add(product.toMap());
    print(doc);
  }
  Future<List<Product>> getAllCash() async {
    String myid = FirebaseAuth.instance.currentUser.uid;
    QuerySnapshot<Map<String, dynamic>> allProductsSnapshot =
    await firebaseFirestore.collection('users').doc(myid).collection('cash').get();
    print("mmmmm");
  ...
  deleteCash(String productId) async {
    String myid = FirebaseAuth.instance.currentUser.uid;
    await firebaseFirestore.collection('users').doc(myid).collection('cash').doc(productId).delete();
  }
